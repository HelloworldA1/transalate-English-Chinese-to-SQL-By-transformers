...
def attention(Q, K, V, mask):
    ...
    score = torch.matmul(Q, K.permute(0, 1, 3, 2))#注意力矩阵
...
class MultiHead(torch.nn.Module):
    ...
    def forward(self, Q, K, V, mask):
        ...
        # 规范化(<<Attention is all you need>>文中，规范化放在后面进行计算;现认为规范化层放在前面会更好，有助于模型收敛)
        Q = self.norm(Q)
        K = self.norm(K)
        V = self.norm(V)
...
class PositionEmbedding(torch.nn.Module):
    def __init__(self):
        ...
        # 词编码层
        self.embed = torch.nn.Embedding(max(len(word_dict),len(tag_dict)), 32)
        # 初始化参数
